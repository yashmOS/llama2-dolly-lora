...
def build_response_anchor(tok, normalized_template: str) -> Union[str, List[int]]:
    """
    Robust anchor for TRL's DataCollatorForCompletionOnlyLM:
      ...
      4) If result is empty, fall back to string anchor '### Response:'.
    """
...
def main(cfg_path: str):
    ...
    # Normalize YAML template (remove indentation / leading blank)
    ...
    # Model (QLoRA 4-bit) — NOTE: no device_map="auto" under FSDP.
    bnb = build_bnb(cfg)
    if bnb is not None:
        raise ValueError(
            "QLoRA (bitsandbytes) is not supported with FSDP in this script. "
            "Set quantization.qlora_4bit: false in r3_fsdp_scaling.yaml."
        )
    ...
    model.config.use_cache = False  # important whenever checkpointing is used
    if bnb is not None:
    ...
    # Training args — IMPORTANT: gradient_checkpointing=False (conflicts with FSDP activation_checkpointing)
    ta = TrainingArguments(
        ...
        #FSDP does activation checkpointing via accelerate YAML
        ...
        # Load best-at-end so we keep the best epoch
        ...
        # Avoid unused-parameters overhead in distributed modes
